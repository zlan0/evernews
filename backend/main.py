...
async def trigger_scrape(req: ScrapeRequest = ScrapeRequest(), _=Depends(verify_admin)):
    """Manually trigger RSS scrape cycle from admin panel."""
    import threading
    result = {"error": None, "new_count": 0}

    def do_scrape():
        try:
            import sys
            sys.path.insert(0, os.path.dirname(__file__))
            from rss_scraper import scrape_all_feeds, RSS_FEEDS
            custom_feeds = req.feeds if req.feeds else None
            count = scrape_all_feeds(auto_publish_override=req.auto_publish, custom_feeds=custom_feeds)
            result["new_count"] = count or 0
        except Exception as e:
            result["error"] = str(e)
            log.error(f"Scrape thread error: {e}")

    t = threading.Thread(target=do_scrape, daemon=True)
    t.start()
    t.join(timeout=90)  # wait up to 90s
    return {"ok": result["error"] is None, "error": result["error"], "new_articles": result["new_count"]}
