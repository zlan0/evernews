...
def scrape_all_feeds(auto_publish_override: bool = None, custom_feeds: list = None):
    log.info("Starting RSS scrape cycle...")
    new_count = 0
    # Allow override from API call; fallback to env var
    if auto_publish_override is not None:
        status = "published" if auto_publish_override else "pending"
    else:
        status = "published" if AUTO_PUBLISH_SCRAPES else "pending"

    feeds_to_use = custom_feeds if custom_feeds else RSS_FEEDS
    for feed_meta in feeds_to_use:
        try:
